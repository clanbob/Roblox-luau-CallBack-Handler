...
- [Status API examples](#status-api-examples)
- [Heap stability test harness](#heap-stability-test-harness)
...
### `events:GetStatus()`
Returns aggregated counters for this dispatcher:
...
### `playerEvents:GetStatus()`
Returns aggregated counters across all player registries:

- `is_player_dispatcher`
- `registries`
- `groups`
- `active_listeners`
- `pending_listeners`
...
### `instEvents:GetStatus()`
Returns aggregated counters across all instance registries:
...
Use `GetStatus()` to programmatically confirm listener/registry cleanup over time.

### Global dispatcher status

```lua
local events = CallBack.New()

events:Listen("Status.Example", false, function() end)

print("before fire", events:GetStatus())
-- { groups = 1, active_listeners = 1, pending_listeners = 0, total_listeners = 1, destroyed = false }

events:Fire("Status.Example")
events:Destroy()

print("after destroy", events:GetStatus())
-- { groups = 0, active_listeners = 0, pending_listeners = 0, total_listeners = 0, destroyed = true }
```

### Instance/player dispatcher status

```lua
local instEvents = CallBack.NewByInstance()

instEvents:Listen(workspace.Baseplate, "Status.Example", false, function() end)

print("before destroy", instEvents:GetStatus())
-- { is_player_dispatcher = false, registries = 1, groups = 1, ... , destroyed = false }

instEvents:Destroy()

print("after destroy", instEvents:GetStatus())
-- { is_player_dispatcher = false, registries = 0, groups = 0, ... , destroyed = true }
```


---

## Heap stability test harness

Use this server-side harness in Studio to validate that memory usage stabilizes over repeated create/fire/destroy cycles.

### How to run

1. Create a temporary `Script` under `ServerScriptService`.
2. Paste the code below.
3. Start Play Solo (server context).
4. Watch memory in **View -> Stats** (or Developer Console).
5. Let it run for several minutes.

Expected result: memory should form a bounded sawtooth/plateau pattern after warmup, not a steady upward trend.

```lua
local CallBack = require(path.To.CallBack)

local CYCLES = 250
local LISTENERS_PER_GROUP = 200
local FIRES_PER_CYCLE = 20

local function runGlobalCycle(cycle)
	local events = CallBack.New()

	for i = 1, LISTENERS_PER_GROUP do
		events:Listen("Heap.Stress", false, function(value)
			-- lightweight callback; keep deterministic
			local _ = value + i
		end)
	end

	for n = 1, FIRES_PER_CYCLE do
		events:Fire("Heap.Stress", n)
	end

	events:Destroy()

	if cycle % 25 == 0 then
		print("[CallBack stress] cycle", cycle, "completed")
	end
end

task.spawn(function()
	for cycle = 1, CYCLES do
		runGlobalCycle(cycle)
		task.wait(0.03)
	end

	print("[CallBack stress] done")
end)
```

### Optional instance-scoped stress

This variant exercises instance registry creation and automatic cleanup paths:
...
task.spawn(function()
	for cycle = 1, CYCLES do
		...
		folder.Name = "HeapStress_" .. cycle
		folder.Parent = workspace

		for i = 1, LISTENERS do
			instEvents:Listen(folder, "Instance.Stress", false, function(v)
				local _ = v * i
			end)
		end

		instEvents:Fire(folder, "Instance.Stress", cycle)
		folder:Destroy() -- triggers registry cleanup via destroy signal
		instEvents:Destroy()

		if cycle % 25 == 0 then
			print("[CallBack instance stress] cycle", cycle, "completed")
		end

		task.wait(0.03)
	end

	print("[CallBack instance stress] done")
end)
```

### Interpreting results

- **Healthy:** memory rises/falls in a range and stabilizes near a baseline after warmup.
- **Suspicious:** memory increases across many identical cycles without settling.
- If suspicious, compare A/B builds and check that registry/connection counts return to near-baseline each cycle.
- During stress runs, print `dispatcher:GetStatus()` every 20-50 cycles and confirm counters return close to zero after destroy phases.
- Also verify `destroyed` flips to `true` after explicit dispatcher `:Destroy()`.
